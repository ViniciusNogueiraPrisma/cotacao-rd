...
$(document).ready(function () {
    ...
    setTimeout("$('div#eventos div').each(function () {  var id = $(this).attr('id');  var count = $(this).find('ul').children().length;  var titulo = $(this).find('span.title').first().text().toLowerCase();  var dataFim = '0';  if($(this).attr('datafim') != ''){  dataFim = $(this).attr('datafim').split('/')[0];  }  $('.ui-datepicker-calendar a').each(function () {  var dia = $(this).text();  if (dia == id || dia == dataFim) {  if (titulo.indexOf('call') != -1)  {  $(this).addClass('blue-dark');  }  else if (titulo.indexOf('divulgação') != -1 || titulo.indexOf('release') != -1) {  $(this).addClass('green');  }   else if (titulo.indexOf('conferência') != -1 || titulo.indexOf('conference') != -1) {  $(this).addClass('blue');  }  else if (titulo.indexOf('assembleia') != -1 || titulo.indexOf('meetings') != -1 || titulo.indexOf('ago') != -1) {  $(this).addClass('roxo');  } else {  $(this).addClass('orange');}}});});", 1000);
...
function waitFunction() {
    ...
    setTimeout("$('.ui-datepicker-prev').show();", 1000);
    setTimeout("$('.ui-datepicker-next').show();", 1000);
...
function excluirEventoDaListEnviarLembrete(e) {
    ...
    var splitCookieEvento = docCookies.getItem('CookieListaAgendaRD').split(',');
    
    spliSinqiaookieEvento = $.grep(splitCookieEvento, function (value) {
        return value != excluirId;        
    });
    docCookies.setItem('CookieListaAgendaTC', splitCookieEvento);
        
    var splitCookieLembrete = docCookies.getItem("CookieEnviarLembreteAgendaRD").split(',');
    ...
    for (var i = 0; i < splitCookieLembrete.length; i++) {
        if (splitCookieLembrete[i].split('&D')[0] == excluirId) {
            ...
            docCookies.setItem('CookieEnviarLembreteAgendaRD', splitCookieLembrete);
        }
    }
...
function addEventoAListEnviarLembrete(e) {
    ...
    if (e.currentTarget.checked == true) {
        ...
        var cookieEvento = docCookies.getItem('CookieListaAgendaRD');
        ...
        if (typeof cookieEvento != 'undefined' && cookieEvento != "" && cookieEvento != null) { // Se cookie já tiver valor
            ...
            if ($.inArray(idEvento, splitCookieEvento) == -1) // Se idEvento NÃO estiver no cookie
            {
                ...
                docCookies.setItem('CookieListaAgendaRD', splitCookieEvento);

                var cookieLembrete = docCookies.getItem("CookieEnviarLembreteAgendaRD");  //  Adiciona o cookie
                ...
                docCookies.setItem("CookieEnviarLembreteAgendaRD", splitCookieLembrete);  //  evento
            ...
            docCookies.setItem("CookieListaAgendaTC", idEvento);
            docCookies.setItem("CookieEnviarLembreteAgendaRD", idEvento + '&D' + diasParaLembrete);
...
function MarcarLembrete() {
    ...
    var getCookieLembrete = docCookies.getItem('CookieEnviarLembreteAgendaRD').split(',');
